	...
	<div id="navbarCollapse" class="collapse navbar-collapse justify-content-start">		
		...
		<ul class="nav navbar-nav navbar-right ml-auto">
           ...
           <li class="nav-item"><a href="mailto:labourfriend134@gmail.com" class="nav-link"><i class="fa fa-envelope"></i><span>Contact Us</span></a></li>	
            ...
            <li class="nav-item"><a href="change-username.php" class="nav-link"><i class="fa fa-key" aria-hidden="true"></i><span>Change Username</span></a></li>
...
<div class="header">
  ...
  <form method="post" action="" enctype="multipart/form-data">
    ...
    <?php endif ?>
        ...
        <tr><td><input type="text" name="password_1" placeholder="Enter current password" style="height:45px;color:black" required></td></tr>
		<tr><td>New Password:</td></tr> 
        <tr><td><input type="text" name="password_2" placeholder="Enter new password" style="height:45px;color:black" required></td></tr>
        <tr><td>Confirm New Password:</td></tr> 
        <tr><td><input type="password" name="password_3" placeholder="Confirm new password" style="height:45px;color:black" required></td></tr>
...
if(isset($_POST['update'])) // Get id
{
  ...
  $sqluser="SELECT hirer_username FROM outside WHERE hirer_username='$username_1' ";
...
{echo '<script language="javascript">alert("Current username is wrong.");</script>';}
  ...
  $sqluser="SELECT password FROM outside WHERE password='$password_1' ";
  $qresult=mysqli_query($db, $sqluser);
  $count=mysqli_num_rows($qresult);
   if($count == 0)
{echo '<script language="javascript">alert("Current password is wrong.");</script>';}
  ...
  if (empty($password_3)) { array_push($errors, "Confirm password is required"); }
  if ($password_2 != $password_3) 
	   {
	 array_push($errors, "The two password did not match");
  
	  {echo '<script language="javascript">alert("New password & confirm new password did not match.");</script>';}
	   }
 if(count($errors) == 0){
	 
$sql="UPDATE `outside` SET password='$password_3' WHERE hirer_username='$username_1' AND password = '$password_1'";	
